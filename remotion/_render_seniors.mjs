// Render do job SENIORS (Stage 4). Uso: node _render_seniors.mjs [ini_s] [fim_s]
...
const BUNDLE = resolve("_bundle_seniors");
const TMP = resolve("_tmp_seniors");
mkdirSync(TMP, { recursive: true });
mkdirSync(resolve("out/_seniors"), { recursive: true });
process.env.TMP = process.env.TEMP = process.env.TMPDIR = TMP;

const mont = JSON.parse(readFileSync(resolve("public/jobs/seniors_mont/montagem.json"), "utf-8"));
...
const out = ini === 0 && fim >= mont.dur_s ? "out/_seniors/seniors_full.mp4" : `out/_seniors/seniors_${ini}_${fim}.mp4`;
...
const comp = await selectComposition({ serveUrl, id: "Montagem", inputProps: { job: "seniors_mont", mont } });
console.log(`dur total: ${comp.durationInFrames}f`);

await renderMedia({
  composition: comp, serveUrl, codec: "h264",
  outputLocation: resolve(out), inputProps: { job: "seniors_mont", mont },
