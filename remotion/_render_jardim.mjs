...
const BUNDLE = resolve("_bundle_jardim");
const TMP = resolve("_tmp_jardim");
mkdirSync(TMP, { recursive: true });
mkdirSync(resolve("out/_jardim"), { recursive: true });
process.env.TMP = process.env.TEMP = process.env.TMPDIR = TMP;

const mont = JSON.parse(readFileSync(resolve("public/jobs/jardim_mont/montagem.json"), "utf-8"));
...
const out = ini === 0 && fim >= mont.dur_s ? "out/_jardim/jardim_full.mp4" : `out/_jardim/jardim_${ini}_${fim}.mp4`;
...
const comp = await selectComposition({ serveUrl, id: "Montagem", inputProps: { job: "jardim_mont", mont } });
console.log(`dur total: ${comp.durationInFrames}f`);

await renderMedia({
  composition: comp, serveUrl, codec: "h264",
  outputLocation: resolve(out), inputProps: { job: "jardim_mont", mont },
