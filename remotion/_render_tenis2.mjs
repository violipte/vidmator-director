...
const BUNDLE = resolve("_bundle_tenis2");
const TMP = resolve("_tmp_tenis2");
mkdirSync(TMP, { recursive: true });
mkdirSync(resolve("out/_tenis2"), { recursive: true });
process.env.TMP = process.env.TEMP = process.env.TMPDIR = TMP;

const mont = JSON.parse(readFileSync(resolve("public/jobs/tenis2_mont/montagem.json"), "utf-8"));
...
const out = ini === 0 && fim >= mont.dur_s ? "out/_tenis2/tenis2_full.mp4" : `out/_tenis2/tenis2_${ini}_${fim}.mp4`;
...
const comp = await selectComposition({ serveUrl, id: "Montagem", inputProps: { job: "tenis2_mont", mont } });
console.log(`dur total: ${comp.durationInFrames}f`);

await renderMedia({
  composition: comp, serveUrl, codec: "h264",
  outputLocation: resolve(out), inputProps: { job: "tenis2_mont", mont },
