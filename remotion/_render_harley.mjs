// Render do job HARLEY (Stage 4). Uso: node _render_harley.mjs [ini_s] [fim_s]
// Isolado por JOB: bundle/tmp ABSOLUTOS próprios (regra nicho automotivo).
import { bundle } from "@remotion/bundler";
import { renderMedia, selectComposition } from "@remotion/renderer";
import { resolve } from "path";
import { mkdirSync, readFileSync } from "fs";

const BUNDLE = resolve("_bundle_harley");
const TMP = resolve("_tmp_harley");
mkdirSync(TMP, { recursive: true });
mkdirSync(resolve("out/_harley"), { recursive: true });
process.env.TMP = process.env.TEMP = process.env.TMPDIR = TMP;

const mont = JSON.parse(readFileSync(resolve("public/jobs/harley_mont/montagem.json"), "utf-8"));
...
const out = ini === 0 && fim >= mont.dur_s ? "out/_harley/harley_full.mp4" : `out/_harley/harley_${ini}_${fim}.mp4`;
...
const comp = await selectComposition({ serveUrl, id: "Montagem", inputProps: { job: "harley_mont", mont } });
console.log(`dur total: ${comp.durationInFrames}f`);

await renderMedia({
  composition: comp, serveUrl, codec: "h264",
  outputLocation: resolve(out), inputProps: { job: "harley_mont", mont },
