...
const BUNDLE = resolve("_bundle_bikes");
const TMP = resolve("_tmp_bikes");
mkdirSync(TMP, { recursive: true });
mkdirSync(resolve("out/_bikes"), { recursive: true });
process.env.TMP = process.env.TEMP = process.env.TMPDIR = TMP;

const mont = JSON.parse(readFileSync(resolve("public/jobs/bikes_mont/montagem.json"), "utf-8"));
...
const out = ini === 0 && fim >= mont.dur_s ? "out/_bikes/bikes_full.mp4" : `out/_bikes/bikes_${ini}_${fim}.mp4`;
...
const comp = await selectComposition({ serveUrl, id: "Montagem", inputProps: { job: "bikes_mont", mont } });
console.log(`dur total: ${comp.durationInFrames}f`);

await renderMedia({
  composition: comp, serveUrl, codec: "h264",
  outputLocation: resolve(out), inputProps: { job: "bikes_mont", mont },
